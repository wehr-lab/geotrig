...
keepidx=find(pause_durs>=pause_winsize);
pause_start_frames=pause_start_frames(keepidx);
pause_end_frames=pause_end_frames(keepidx);
pause_durs=pause_durs(keepidx);
fprintf('\nafter excluding pauses <%d frames, kept %d pauses, min duration %d frames (mean %.0f)', pause_winsize, length(pause_durs),  min(pause_durs), mean(pause_durs))
